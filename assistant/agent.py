...
from assistant.toolkit.tool_loader import load_all_tools 
...
class LLMProcessor:
    ...
    def __init__(self):
        ...
        logger.info(f"LLM Processor initialized with {len(self.tools)} tools.")
    ...
    def _call_tool(self, state: AgentState) -> dict:
        ...
        for tool_call in tool_calls:
            ...
            logger.info(f"Using tool: {tool_name} with args: {tool_call['function']['arguments']}")
            ...
            try:
                ...
                output = tool_to_call.invoke(tool_call["function"]["arguments"])
    ...
    def process(self, text: str) -> Generator[str, None, None]:
        ...
        """
        system_message = SystemMessage(content="You are a helpful voice assistant. Be concise and don't use markdown formatting like _italics_, **bold** and `code`.")
        full_history = [system_message, HumanMessage(content=text)]
        
        # Use .stream() to get a generator of intermediate steps from the graph.
        stream = self.graph.stream({"messages": full_history})
        
        for chunk in stream:
            # The stream yields the output of the node that just ran.
            # We check if it's the final LLM response.
            if "llm" in chunk:
                last_message = chunk["llm"]["messages"][-1]
                
                # If it's a final answer (no tool calls), we stream it.
                if "tool_calls" not in last_message.additional_kwargs:
                    # We use the accumulated history to make a valid streaming call.
                    for token in self.llm.stream(full_history):
                        yield token.content
                    # Once we start streaming the final answer, we can exit.
                    return
                else:
                    # It's a tool call, so we add it to our history and continue.
                    full_history.extend(chunk["llm"]["messages"])
            
            elif "tools" in chunk:
                # It's a tool response, so we add it to our history and continue.
                full_history.extend(chunk["tools"]["messages"])
